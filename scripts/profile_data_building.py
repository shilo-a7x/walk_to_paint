...
def profile_data_creation(
    dataset_name, num_walks_list, max_walk_length_list, output_file=None
):
    """
    ...
    for num_walks in num_walks_list:
        for max_walk_length in max_walk_length_list:
            current += 1
            print(
                f"[{current}/{num_params}] Profiling num_walks={num_walks:,}, max_walk_length={max_walk_length}..."
            )
    ...
    for r in results:
        print(
            f"{r['num_walks']:>15,} {r['max_walk_length']:>10} {r['walk_sampling_time']:>10.2f} {r['walks_per_second']:>12.0f}"
        )
    ...
    if under_2min:
        ...
        for r in sorted(under_2min, key=lambda x: -x["num_walks"])[:3]:
            print(
                f"   {r['num_walks']:,} walks, length={r['max_walk_length']}: {r['walk_sampling_time']:.2f}s"
            )
...
def estimate_preprocessing_time(
    dataset_name, num_walks, max_walk_length, profiling_data=None
):
    """
    ...
    if profiling_data is None:
        ...
        profiling_data = profile_data_creation(
            dataset_name, [num_walks], [max_walk_length]
        )
