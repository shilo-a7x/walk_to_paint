...
class EvaluationPipeline:
    ...
    def load_model(self, device=2):
        ...
        if not matches:
            raise FileNotFoundError(f"No checkpoint found for trial {self.best_trial.number}")
        ...
        # Load model
        self.model = LitEdgeClassifier.load_from_checkpoint(str(ckpt_path), config=self.cfg)
        self.model.eval()
