...
class OptunaBestModel:
    ...
    def __init__(self, dataset_name, outputs_root="/home/dsi/shilo_avital/yolo_lab/walk_to_paint/outputs"):
        """
        Args:
    ...
    def find_best_study(self):
        ...
        print(f"   Best trial: #{self.best_trial.number} with score {self.study.best_value:.6f}")
    ...
    def prepare_data(self, use_cache=True, cache_dir=None):
        ...
        if use_cache and cache_dir is None:
            cache_dir = Path(self.outputs_root) / self.dataset_name / "preprocessed_cache"
            cache_dir.mkdir(parents=True, exist_ok=True)
        
        # Prepare data (this will use cache if available)
        # You'll need to add caching logic to prepare_data
        train_loader, val_loader, test_loader = prepare_data(cfg, cache_dir=cache_dir if use_cache else None)
...
def resume_optuna_study(dataset_name, n_more_trials=50, outputs_root="/home/dsi/shilo_avital/yolo_lab/walk_to_paint/outputs"):
    """
