...
def extract_edge_scores_from_dataloader(model, dataloader, tokenizer, device, stage_name, cfg):
    """
        ...
        for batch in tqdm(dataloader, desc=f"Processing {stage_name}"):
            ...
            for b in range(batch_size):
                for i in range(1, seq_len - 1):  # Skip boundaries
                    ...
                    try:
                        u_token = tokenizer.id2token.get(input_ids[b, i-1].item(), None)
                        v_token = tokenizer.id2token.get(input_ids[b, i+1].item(), None)
...
def main():
    ...
    def build_feature_dict(edge_scores_dict):
        ...
        result = {}
        for edge_key, scores in tqdm(edge_scores_dict.items(), desc="Computing features"):
