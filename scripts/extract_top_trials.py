...
def get_alternative_top_trials(dataset_name, outputs_root="/home/dsi/shilo_avital/yolo_lab/walk_to_paint/outputs", 
                               metric="num_walks", tolerance_pct=5.0, top_n=10):
    """
    ...
    for idx, row in df_valid.head(top_n).iterrows():
        ...
        print(f"    num_walks: {row['num_walks']:,.0f}" if pd.notna(row['num_walks']) else "")
        print(f"    max_walk_length: {row['max_walk_length']}" if pd.notna(row['max_walk_length']) else "")
...
def save_trial_details(dataset_name, trial_id, outputs_root="/home/dsi/shilo_avital/yolo_lab/walk_to_paint/outputs"):
        ...
        try:
            ...
            for t in study.trials:
                if t.number == trial_id and t.state.name == "COMPLETE":
                    ...
                    # Save to file
                    output_file = exp_folder / "optuna" / f"trial_{trial_id}_params.json"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python extract_top_trials.py <dataset_name> [--metric num_walks|max_walk_length] [--tolerance 5]")
        print("\nExamples:")
        print("  python extract_top_trials.py wiki-rfa")
        print("  python extract_top_trials.py epinions --metric num_walks --tolerance 2")
    ...
    try:
        result = get_alternative_top_trials(dataset, metric=metric, tolerance_pct=tolerance, top_n=10)
