...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/empconf_panels_abc_combined.png}
\caption{(a) Per-node sign entropy, source out-degree ($\Hh_\outdeg$, rater consistency) vs.\ target in-degree ($\Hh_\indeg$, reputation contestedness), six datasets; box = IQR, diamond = mean (the median and IQR collapse to exactly 0 on four datasets due to a high fraction of unanimous nodes -- the mean marker is the informative summary there), whiskers to $1.5\times$IQR, one-sided Wilcoxon signed-rank test direction and significance annotated per dataset (ties dropped). (b) Normalized mutual information ($\mathrm{NMI} = \tfrac{\MI}{\Hh(Y)}$, comparable across datasets with different label imbalance) between an anchor edge's sign and a context edge's sign, as a function of line-graph distance between them (1 = share a node; each further unit is one more undirected hop between their nearest endpoints), six datasets, linear scale. (c) Test AUC (discrete 4$\times$4 bins) for two GNN baselines, SGA-augmented SiGAT and GINEConv, as a function of source out-entropy ($\Hh_\outdeg$ of the source vertex, rater consistency) and target in-entropy ($\Hh_\indeg$ of the target vertex, reputation contestedness), six datasets.}
...
SiGAT (SGA) & $0.882_{\pm.010}$ & $0.876_{\pm.007}$ & $0.915_{\pm.001}$ & $0.893_{\pm.003}$ & $0.883_{\pm.003}$ & $0.859_{\pm.002}$ \\
...
\caption{Test AUC (kernel-smoothed over entropy space, not discrete bins) for \method\ (full attention), \method\ (LocalAttn4), and two GNN baselines (SGA-augmented SiGAT, GINEConv), as a function of source out-entropy (rater consistency) and target in-entropy (reputation contestedness), six datasets. The two GNN rows are the same underlying data as Empirical Confirmation Figure~\ref{fig:empconf-panels}c (there shown as discrete 4$\times$4 bins; here kept as the original kernel-smoothed surface for a finer-grained comparison against \method's own smoothed rows), repeated here so this figure stands alone as a complete walk-vs-GNN comparison.}
\label{fig:result2-heatmap}
\end{figure}

\subsection{Result 3: \method\ and the direction of information flow}
\ph{PLACEHOLDER: [OPEN QUESTION, LOWER PRIORITY -- deprioritized 2026-07-20] Whether \method's
attention weights carry a directional signal related to the source/target entropy asymmetry is
not yet resolved and is not being actively pursued at the moment. One candidate, not yet designed
or run: compare the attention mass a masked target-edge token places on tokens belonging to the
source node $u$ versus tokens belonging to the target node $v$, and check whether this differs on
wiki-elec/wiki-rfa (the two datasets where the entropy asymmetry itself reverses) from the other
four -- a reversal there would be more convincing than a skew observed only on the confirming
datasets. Whether attention-weight magnitude can be validly read as "information flow" at all is
an open question in its own right; revisit once higher-priority items are done.}
