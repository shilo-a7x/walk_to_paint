...
| 5 | ~~Table 1 area, line ~272~~ | ~~`XXXX ... lets also add Precision Recall AUC or perhaps F1...`~~ | **Resolved 2026-08-23.** Macro-F1 (threshold 0.5) added as a second sub-column per dataset in Table 1 (`table*`, full double-column width has room for AUC+F1 per dataset without restructuring). Pewter (both variants)/SiGAT/SNEA/CopulaLSP macro-F1 computed and verified against real AUC ground truth; node2vec's macro-F1 rerun (`baselines/node2vec/run_node2vec.py`, now also saves `tst_macro_f1`); GCN/GAT/SGCN/GS-GNN F1 from~\citep{huang2019signed}'s own bare (non-SGA-augmented) reproduction, matching how their AUC is already sourced. Bold/second-best re-derived per (dataset, metric) — note bitcoin-alpha's F1 second-best is GS-GNN, not Pewter-full (the only dataset where a baseline breaks into the top 2). PR-AUC stays out (still only 2/9 rows have raw per-edge scores). Marker removed from the tex. |
| 6 | ~~Ablations subsection, line ~331~~ | ~~`XXXX There are three intereseting ablations needed to be checked...`~~ | **Resolved 2026-08-22, numbers corrected 2026-08-23 (later same day).** 180-job campaign finished; all three ablations (DIRFLIP, MASKNODE, MASKEDGE) written into the tex, backed by one-sided paired Wilcoxon tests (10 seeds). MASKNODE/MASKEDGE's first-landed numbers had a real posthoc-eval bug (`src/training/callbacks.py` not reapplying the token-masking ablation at inference); fixed, all 120 checkpoints re-evaluated (no retraining), tex updated with corrected numbers. **MASKEDGE's finding flipped from "small but significant cost" to "no significant effect on any dataset."** Scripts: `scripts/rerun_ablation_posthoc_fix.py` (re-eval), `scripts/paper_figures/extract_ablation_masknode_maskedge_significance.py` (numbers). Data: `aaai2027/figure_data/ablation_masknode_maskedge_significance.csv`. Full story: `PAPER_CLOSEOUT_LOG.md`'s second 2026-08-23 entry, `TRIVIAL_BASELINE_INVESTIGATION.md`. Marker removed. |
