...
We use \eqref{eq:invfano} rather than the more familiar linear corollary $P_e\ge(\Hh(Y\mid Z)-1)/\log c$ because at $c=2$ the latter is vacuous: $\log c=1$ and $\Hh(Y\mid Z)\le 1$ bit, so its right-hand side is never positive. 
...
\label{as:structural}
The endpoints enter the read-out only through their WL colors $\chi(u),\chi(v)$, and these colors are not injective on the edges we predict (In simple terms, there is a positive, and in practice non-negligible probability that a WL color pair is shared by two edges of different labels.
\end{assumption}
...
XXXX THE ENTIRE FOLLOWING PARAGRAPH IS UNCLEAR AND SHOULD BE REWRITTEN XXXXX
...
\centering
\scriptsize
\caption{Per-dataset size and walk budget. $\kappa$ is the walk-budget multiplier, $M=\kappa|E|$, fixed per dataset along with the rest of the per-dataset configuration (Setup below).}
...
XXXXXX detail all the statistical tests that you do, and please add such tests. xxxxx
...
XXXXX  PLEASE DEFINE HERE CLEARLY THE DIFFERENT TYPES OF ENTROPY WITH AN EXAMPLE, INCLUDING THE ENUMERATION ABOVE. THEN GIVE THE PRIOR ARGUMENT WHY WE EXPECT IT TO DECREASE.
...
XXXXX PLEASE HAVE C TO BE TWO ROWS AND LARGER BOXES XXXXXX
XXXX PLEASE ALSO HAVE A STATISTICAL TEST PER DATASET ON THE EFFECT OF SOURCE AND TARGET ENTROPY AND THEIR JOINED EFFECT (A SIMPLE TWO WAY ANOVA - YOU CAN USE THE CROSS VALIDATIONS AS MULTIPLE MEASUREMENTS) XXXXX
...
XXXX IN D please replace the bars by dots for each dataset - please keep the colors consistent for datasets all along. In e, please do stacked bar plots. ALSO PLEASE HAVE A,B,C,D,... IN CAPITAL. also please have statistical tests for D and e - I will write the text after you fix that, but please do adapt the caption.XXXX
...
\emph{Setup.} A message-passing GNN with any finite number $T$ of rounds assigns to each vertex a color $\chi_T(\cdot)$ that refines to at most the stable WL coloring, and the vertex representation is a fixed function of that color, $h_u=\psi(\chi_T(u))$~\cite{xu2018powerful,morris2019weisfeiler}. Hence any edge read-out satisfies $\hat y_{uv}=g(\psi(\chi_T(u)),\psi(\chi_T(v)))=\tilde g(\chi_T(u),\chi_T(v))$, so $\hat Y$ is measurable with respect to the $\sigma$-algebra $\mathcal{F}$ generated by the color pair $(\chi_T(U),\chi_T(V))$.
