...
Multiple web-based graphs put labels (signs) on edges, not on vertices. A buyer rates a seller as trustworthy or not on a peer-to-peer marketplace, a reviewer marks another reviewer as trusted or distrusted on a product review site, and an editor supports or opposes a candidate in an adminship election. In each case a directed edge carries a sign, most edges are unsigned or unobserved, and the practical task is to predict the missing signs from the observed ones. 
...
\]
XXXXX THIS BOUND IS BASICALLY VACUOUS: $\Hh_b^{-1}$ SATURATES AT $\tfrac12$, SO THE RHS IS ONLY ABOVE $0$ ONCE $\Hh(Y)-2\log N$ CLEARS $0$, MEANING $2\log N$ HAS TO BE CLOSE TO $\Hh(Y)\le1$ BIT -- REAL EMBEDDING WIDTHS MAKE $N$ WAY TOO LARGE FOR THAT, SO THIS RARELY BINDS XXXXX
...
\subsection{\method\ is more accurate than SOTA}

\method\ treats edge-sign prediction as a per-walk classification task, not a per-vertex embedding lookup. For a target edge, we sample many directed walks through it, tokenize each walk as an alternating sequence of vertex and edge-sign tokens, hide the target edge's sign, and score it with a Transformer trained to recover the hidden sign from the surrounding walk context (Figure~\ref{fig:pipeline-schematic}, Stage 1). Because a given edge typically appears in many sampled walks, this yields many independent per-walk probability estimates for the same edge, which a validation-tuned confidence-weighted vote pools into one edge-level prediction (Figure~\ref{fig:pipeline-schematic}, Stage 2). We evaluate two attention variants over the walk, full and local (\S\ref{sec:methods}), and report each dataset's better variant. Unlike the vertex-centric baselines below, every prediction is conditioned directly on nearby edge signs, not only on the two endpoints' learned representations.
...
\method\ has a higher test AUC than the best baseline on all six datasets, under both attention variants (Table~\ref{tab:result1}), by a mean of $+3.3$ AUC points and a range of $+1.4$ (Wiki-elec, Wiki-RfA) to $+4.5$ (Epinions) points, comparing each dataset's best \method\ variant against its best baseline; a one-sided paired Wilcoxon signed-rank test across the 10 splits confirms \method\ wins on all 10 splits on every dataset ($p=0.00098$ throughout).
...
\midrule
\method\ (full attention) & $\mathbf{0.915_{\pm.010}}$ & $\mathit{0.931_{\pm.007}}$ & $\mathit{0.952_{\pm.001}}$ & $\mathit{0.901_{\pm.003}}$ & $\mathbf{0.892_{\pm.003}}$ & $\mathbf{0.899_{\pm.002}}$ \\
\method\ (local attention) & $\mathit{0.913_{\pm.017}}$ & $\mathbf{0.932_{\pm.007}}$ & $\mathbf{0.954_{\pm.002}}$ & $\mathbf{0.902_{\pm.003}}$ & $\mathit{0.891_{\pm.005}}$ & $\mathit{0.897_{\pm.002}}$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\subsection{Where \method's attention goes}
...
\emph{Setup.} Let each embedding take at most $N$ distinguishable values under the read-out.

\emph{Bound.} A variable supported on $\le N$ values has entropy $\le \log N$, so $\MI(Y;h_U,h_V)\le \Hh(h_U,h_V)\le \Hh(h_U)+\Hh(h_V)\le 2\log N$. Substituting into $\Hh(Y\mid h_U,h_V)=\Hh(Y)-\MI(Y;h_U,h_V)$ gives $\Hh(Y\mid h_U,h_V)\ge \Hh(Y)-2\log N$. Applying \eqref{eq:invfano} and using that $\Hh_b^{-1}$ is increasing, $P_e\ge \Hh_b^{-1}\big(\Hh(Y)-2\log N\big)$; with the convention $\Hh_b^{-1}(x):=0$ for $x\le0$ this is unconditionally true, but informative only when $\Hh(Y)>2\log N$.

\emph{Per-vertex specialization.} $h_U$ alone satisfies $\MI(\{Y_e\}_{e\ni U}; h_U)\le \Hh(h_U)\le \log N$ by the same argument. If $U$ has out-degree $D$ with near-independent out-edge labels of entropy $\Hh_\outdeg(U)$ each, they carry $D\,\Hh_\outdeg(U)$ bits jointly; once this exceeds $\log N$ the excess cannot pass through the shared embedding. $\square$
