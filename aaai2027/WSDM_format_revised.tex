...
XXXX There are three intereseting ablations needed to be checked A) randomize the edge direction and see what we get, B) remove the vertex token, and see how much we can learn from edges (or simply set the vertex token to 0). C) remove the edge token, to see if walks can learn from vertices, XXXX 
