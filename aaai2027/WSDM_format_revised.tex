...
\centering
\scriptsize
\caption{Test AUC $\pm$ Hanley--McNeil SE on the held-out test split, for every baseline vs.\ Pewter under both attention variants. Bold marks the best result per dataset, italic the second best. Dashes mark runs that did not complete (Baselines).}
\label{tab:result1}
...
SiGAT & $0.882_{\pm.010}$ & $0.876_{\pm.007}$ & $\mathit{0.915_{\pm.001}}$ & $\mathit{0.893_{\pm.003}}$ & $\mathit{0.883_{\pm.003}}$ & $\mathit{0.859_{\pm.002}}$ \\
\ph{TODO: add a line-graph GNN row} & -- & -- & -- & -- & -- & -- \\
\ph{TODO: add a local-majority heuristic row} & -- & -- & -- & -- & -- & -- \\
\midrule
Pewter (full attention) & $\mathbf{0.922_{\pm.007}}$ & $\mathbf{0.936_{\pm.004}}$ & $0.953_{\pm.001}$ & $0.905_{\pm.003}$ & $0.894_{\pm.002}$ & $\mathbf{0.900_{\pm.001}}$ \\
Pewter (local attention) & $\mathit{0.919_{\pm.007}}$ & $\mathbf{0.936_{\pm.004}}$ & $\mathbf{0.954_{\pm.001}}$ & $\mathbf{0.908_{\pm.003}}$ & $\mathbf{0.898_{\pm.002}}$ & $\mathit{0.898_{\pm.001}}$ \\
\bottomrule
\end{tabular}
\end{table*}
