...
Let $L$ be the maximum walk length in hops, so each walk is a token sequence of length at most $2L+1$; sampling one walk costs $O(L)$, and the per-walk transformer costs $O(L^2d)$ under full attention or $O(Lwd)$ under a genuinely windowed implementation of the local window -- both are properties of a single walk, independent of how many walks are later combined for any one edge. Scoring one already-specified target edge aggregates over at most $M$ walk occurrences, the per-dataset walk budget already fixed in Table~\ref{tab:dataset-walks}; each occurrence contributes one $O(L^2d)$ (or $O(Lwd)$) transformer pass, and combining the (at most) $M$ scores costs $O(M)$ more, since every aggregator function in Ablation~B is a closed form evaluated once per walk, with its one or two scalar parameters fit once per dataset offline rather than refit per edge. Classifying one target edge therefore costs $O(ML^2d)$ under full attention or $O(MLwd)$ under a windowed implementation, independent of $|V|$ and $|E|$ once $M$ and $L$ are fixed. This per-edge cost is separate from the cost of covering a whole graph: the coverage guarantee requires a total sampling budget $M=\kappa|E|$ for a small dataset-chosen constant $\kappa\in[1,5]$ (Setup below), so total corpus-construction cost is $O(|E|)$ up to $\kappa$.

\section{Discussion and Limitations}
\label{sec:limits}
The bounds we prove concern what is conditioned on, so they apply to any vertex-centric read-out rather than to a particular architecture, and they are not removed by rewiring, depth, or width. Pewter sidesteps them by conditioning on signed edges near the target, at the cost of sampling a walk corpus and running a transformer for every walk that contains a queried edge.

Several limitations bound what we can claim. The method assumes signed edges near the target: on an edge whose neighborhood is entirely unsigned, Pewter has nothing to condition on and degrades to the prior, whereas a vertex-centric model can still use a memorized endpoint embedding. Per-edge inference aggregates hundreds to thousands of walk evaluations, so the constant factor is high even though the asymptotic cost is independent of graph size. The two Wikipedia vote graphs behave differently from the four trust graphs on the directionality analysis, and we do not have a mechanism for that beyond the observation that a public vote and a private bilateral rating are different acts; six datasets from two genres is too few to settle it XXX MIGHT NOT BE TRUE WITH NEW RESULTS ON DIRECTIONALITY XXX. Finally, our bound is stated for the structural component of a model, and a fully transductive model with free per-vertex embeddings is outside its scope, as discussed after Assumption~\ref{as:structural}.

\section{Conclusion}
Reading an edge sign off two endpoint embeddings discards information in a way that is predictable from the label entropy that survives WL coarsening, and no depth or width recovers it. Pewter conditions instead on the signed edges around the target, read directly from short walks, and pools across the walks that contain each edge. It improves AUC over eight baselines on six public signed networks, with the largest gains in the high-entropy regime the bound identifies (mean $+3.3$ AUC points over the best baseline per dataset, range $+1.4$ to $+4.5$). Two of our findings are negative and we think worth stating plainly: the direction of edge-sign information is not consistent across dataset genres, and how the per-walk predictions are pooled barely matters.
