...
def create_toy_config(base_config="config.yaml", toy_config="config_toy.yaml"):
    ...
    config["dataset"]["max_walk_length"] = 5  # Very short walks (5 steps = 11 tokens)
    config["dataset"]["num_walks"] = 10000  # Very few walks for super fast testing
...
def main():
    ...
    edges = create_toy_graph(
        num_nodes=30,  # Even smaller - just 30 nodes
        num_edges=100,  # Just 100 edges
        edge_labels=[-1, 0, 1],
    )
