...
def _keyed_cache_path(cfg):
    """Cache filename keyed by (walk_strategy, num_walks, max_walk_length, seed) —
    every distinct sampling config writes its own cache file."""
...
def prepare_data(cfg):
    dataset_cache_path = _keyed_cache_path(cfg)
    ...
    load_path = dataset_cache_path if (
        cfg.preprocess.use_cache and cache_exists(dataset_cache_path)
    ) else None
    ...
    if walk_telemetry:
        # Coverage/dedup stats from edge_cover_walks (see its telemetry_out param):
        # anchor-walk count and fill-phase acceptance/duplication rates.
