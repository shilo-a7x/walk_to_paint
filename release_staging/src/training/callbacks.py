    ...
    def _extract_predictions(self, trainer, pl_module, dataloader, split_name):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(dataloader):
                ...
                # In ragged mode walk_lengths is [B]; expand to [B, S] first.
                _B, _S = labels.shape
        ...
        # dist_from_start is exactly the edge-count position computed above; kept
        # under this name since that's what downstream consumers read.
        result["dist_from_start"] = np.concatenate(all_data["positions"])
